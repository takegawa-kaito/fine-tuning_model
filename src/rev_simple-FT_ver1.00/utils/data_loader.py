...
class RegressionDataset(Dataset):
    ...
    def _clean_data(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 無効な行が見つかった場合の警告（重複防止）
        invalid_count = np.sum(~valid_mask)
        if invalid_count > 0:
            import logging
            logger = logging.getLogger(__name__)
            # データセット作成時のみ警告（評価時の重複警告を防ぐ）
            if not hasattr(self, '_invalid_warning_shown'):
                logger.warning(f"Dataset: Removed {invalid_count} invalid rows "
                              f"(total: {len(valid_mask)}, remaining: {np.sum(valid_mask)})")
                self._invalid_warning_shown = True

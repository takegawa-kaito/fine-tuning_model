...
class Evaluator:
    ...
    def _clean_data_for_analysis(self, X: np.ndarray, y: np.ndarray) -> tuple:
        ...
        if invalid_count > 0:
            self.logger.warning(f"Removed {invalid_count} rows with invalid data during analysis "
                              f"(total rows: {len(valid_mask)}, remaining: {np.sum(valid_mask)})")
            
            # デバッグ情報（詳細ログレベル）
            self.logger.debug(f"Invalid X rows: {np.sum(~X_mask)}, Invalid y rows: {np.sum(~y_mask)}")
